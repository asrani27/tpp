...
class PuskesmasController extends Controller
{
        ...
        try {
            ...
            $data->map(function ($item) {
                ...
                if (Auth::user()->puskesmas->id == 8) {
                    $item->pbk_jumlah = round(($item->pbk_absensi + $item->pbk_aktivitas + $item->pbk_skp) * (68 / 100) * (80 / 100));
                    ...
                    $item->pbk_jumlah = round(($item->pbk_absensi + $item->pbk_aktivitas + $item->pbk_skp) * (85 / 100) * (80 / 100));
                }
                ...
                if (Auth::user()->puskesmas->id == 8) {
                    $item->ppk_jumlah = round(($item->ppk_absensi + $item->ppk_aktivitas + $item->ppk_skp) * (68 / 100) * (80 / 100));
                } elseif (Auth::user()->puskesmas->id == 36 || Auth::user()->puskesmas->id == 37) {
                    $item->ppk_jumlah = round(($item->ppk_absensi + $item->ppk_aktivitas + $item->ppk_skp));
                } else {
                    $item->ppk_jumlah = round(($item->ppk_absensi + $item->ppk_aktivitas + $item->ppk_skp) * (85 / 100) * (80 / 100));
                }

                //PKK
                $item->pkk = $item->basic * ($item->p_kk / 100);
                if (Auth::user()->puskesmas->id == 8) {
                    $item->pkk_jumlah = round($item->pkk * (68 / 100) * (80 / 100));
                } elseif (Auth::user()->puskesmas->id == 36 || Auth::user()->puskesmas->id == 37) {
                    $item->pkk_jumlah = round($item->pkk);
                } else {
                    $item->pkk_jumlah = round($item->pkk * (85 / 100) * (80 / 100));
                }

                //PKP
                $item->pkp = $item->basic * ($item->p_kp / 100);
                if (Auth::user()->puskesmas->id == 8) {
                    $item->pkp_jumlah = round($item->pkp * (68 / 100) * (80 / 100));
                } elseif (Auth::user()->puskesmas->id == 36 || Auth::user()->puskesmas->id == 37) {
                    $item->pkp_jumlah = round($item->pkp);
                } else {
                    $item->pkp_jumlah = round($item->pkp * (85 / 100) * (80 / 100));
                }
    ...
    public function pembayaran($bulan, $tahun)
    {
        ...
        $cuti_bersama = DB::connection('presensi')->table('libur_nasional')->whereMonth('tanggal', $bulan)->whereYear('tanggal', $tahun)->get()->count() * 360;
    ...
    public function cpns($bulan, $tahun)
    {
        ...
        $data = RekapCpns::where('puskesmas_id', Auth::user()->puskesmas->id)->where('bulan', $bulan)->where('tahun', $tahun)->orderBy('kelas', 'DESC')->get();
        $data->map(function ($item) use ($pengali_kedua, $pengali_pertama, $bulan) {
            //PBK
            $item->pbk_absensi = $item->basic * (($item->p_bk + $item->p_tbk) / 100) * (40 / 100) * ($item->dp_absensi / 100);
            if ($bulan == '12') {
                if ($item->dp_ta >= 3375) {
        ...
        foreach ($data as $item) {
            ...
            if ($persen == null) {
                ...
                if (Auth::user()->puskesmas->id == 8) {
                    ...
                    $pagu      = round($basic * (($p_bk + $p_tbk + $p_pk + $p_kk + $p_kp) / 100) * (80 / 100) * (85 / 100));
                }
            }
    ...
    public function puskes_reguler_excel($bulan, $tahun)
    {
        ...
        if ($bulan == '12') {
            ...
            } elseif (Auth::user()->puskesmas->id == 36) {
                $path = public_path('/excel/labkes.xlsx');
            } elseif (Auth::user()->puskesmas->id == 37) {
