...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="row">
            <div class="col-12">
                <a href="/pegawai/skp/validasi" class="btn btn-sm btn-secondary"><i class="fas fa-arrow-alt-circle-left"></i> Kembali</a>
                @if (count($data) != 0)
                <a href="/pegawai/skp/validasi/acc_semua/{{$id}}" class="btn btn-sm btn-success">
                ...
                <div class="card">
                    <div class="card-body table-responsive p-0">
                        ...
                        @foreach ($data as $key => $item)
                                ...
                                <td>{{$item->skp_periode->mulai}} - {{$item->skp_periode->sampai}}</td>
                                ...
                                <td>
                                    @if ($item->validasi == 0)
                                    <a href="/pegawai/skp/validasi/acc/{{$item->id}}" class="btn btn-xs btn-success text-white" data-toggle="tooltip" title="Setujui Aktivitas" onclick="return confirm('Yakin ingin di Setujui?');"><i class="fas fa-check"></i> Setujui</a>
                                    <a href="/pegawai/skp/validasi/tolak/{{$item->id}}" class="btn btn-xs btn-danger text-white" data-toggle="tooltip" title="Tolak Aktivitas"  onclick="return confirm('Yakin ingin di tolak?');"><i class="fas fa-times"></i> Tolak</a>
                                    @else
