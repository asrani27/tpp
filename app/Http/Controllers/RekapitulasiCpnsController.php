    ...
    public function pembayaran($bulan, $tahun)
    {
        ...
        foreach ($data as $item) {
            ...
            if ($jabatan == null) {
                ...
                $bk_disiplin = round((($item->perhitungan_basic_tpp * ($jabatan->persen_beban_kerja + $jabatan->persen_tambahan_beban_kerja - 10) / 100) * ((40 / 100) * $absensi / 100)));
                $bk_produktivitas = round($menit_aktivitas >= 6750 ? ($item->perhitungan_basic_tpp * ($jabatan->persen_beban_kerja + $jabatan->persen_tambahan_beban_kerja - 10) / 100) * 0.6 : 0);
